...
class ElasticSearchBackendTest extends BackendTestBase {
  ...
  protected function searchSuccess() {
    ...
    $results = $this->buildSearch()
      ...
      ->execute();
    $this->assertEquals($this->getItemIds([1, 4, 5]), array_keys($results->getResultItems()), 'Search for created range returned correct results.');

    $results = $this->buildSearch()
      ->addCondition("created", [
        strtotime("2008-06-26"),
        strtotime("2012-06-12"),
      ], "NOT BETWEEN")
      ->execute();
    $this->assertEquals($this->getItemIds([1, 4, 5]), array_keys($results->getResultItems()), 'Search for created range returned correct results.');
    ...
    $results = $this->buildSearch()
      ->addCondition("created",
        ...
        ">")
      ->execute();
    $this->assertEquals($this->getItemIds([3, 4, 5]), array_keys($results->getResultItems()), 'Search for created range returned correct results.');

    $results = $this->buildSearch()
      ->addCondition("created",
        strtotime("2008-06-28"),
        ">")
      ->execute();
    $this->assertEquals($this->getItemIds([3, 4, 5]), array_keys($results->getResultItems()), 'Search for created range returned correct results.');
    ...
    $results = $this->buildSearch()
      ->addCondition("created",
        ...
        "<")
      ->execute();
    $this->assertEquals($this->getItemIds([1, 2]), array_keys($results->getResultItems()), 'Search for created range returned correct results.');

    $results = $this->buildSearch()
      ->addCondition("created",
        strtotime("2008-06-29"),
        "<")
      ->execute();
    $this->assertEquals($this->getItemIds([1, 2]), array_keys($results->getResultItems()), 'Search for created range returned correct results.');
